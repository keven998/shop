                    ...
                    <div class="table-margin">
                        <form action="" class="form-horizontal" method="post">
                            <table class="table table-hover contact-template-form">
                                <tbody>
                                    <?php if(is_array($plugin["config"])): $i = 0; $__LIST__ = $plugin["config"];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$config): $mod = ($i % 2 );++$i;?><tr>
                                            <td width="20%" align="right"><?php echo ($config["label"]); ?>：</td>
                                            <td>
                                                <?php if($config['type'] == 'select'): ?><select name="config[<?php echo ($config['name']); ?>]" class="form-control w-300">
                                                        <?php if(is_array($config["option"])): $o = 0; $__LIST__ = $config["option"];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$option): $mod = ($o % 2 );++$o;?><option <?php if($config_value[$config['name']] == $o): ?>selected<?php endif; ?>  value="<?php echo ($o); ?>"><?php echo ($option); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
                                                    </select>
                                                <?php elseif($config['type'] == 'textarea'): ?>
                                                    <textarea rows="6" cols="90" name="config[<?php echo ($config['name']); ?>]" class="form-control w-300"><?php echo ($config_value[$config['name']]); ?></textarea>
                                                <?php else: ?>
                                                    <input type="<?php echo ($config['type']); ?>" value="<?php echo ($config_value[$config['name']]); ?>" class="form-control w-300" name="config[<?php echo ($config['name']); ?>]"/><?php endif; ?>
                                            </td>
