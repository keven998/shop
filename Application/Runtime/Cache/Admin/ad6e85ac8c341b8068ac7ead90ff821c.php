                            ...
                            <form action="" method="get">
                                <div class="form-group">
                                    ...
                                    <select name="pay_status" id="" class="form-control">
                                        ...
                                        <option value="0" <?php if($_REQUEST['pay_status'] === '0'): ?>selected<?php endif; ?>>待支付</option>
                                        <option value="1" <?php if($_REQUEST['pay_status'] == '1'): ?>selected<?php endif; ?>>已支付</option>
                                    </select>
                                </div>
                                <div class="form-group">
                                    <span class="pull-left form-span">会员ID:</span>
                                    <input type="text" name="user_id" value="<?php echo ($_GET['user_id']); ?>" class="form-control" placeholder="请输入会员ID">
                                </div>
                        ...
                        <table class="table table-hover table-header">
                            ...
                            <?php if($list['data']): ?><tbody>
                                    <?php if(is_array($list["data"])): foreach($list["data"] as $key=>$vo): ?><tr>
                                            ...
                                            <td><a href="<?php echo U('user/detail',array('user_id'=>$vo['user_id']));?>"><?php echo ($vo["user_id"]); ?></a></td>
                                            <td><?php echo (date("Y-m-d H:i",$vo["add_time"])); ?></td>
                                            <td><?php echo ($vo["money"]); ?></td>
                                            <td><?php echo ($vo["bank_name"]); ?></td>
                                            <td><?php echo ($vo["account_bank"]); ?></td>
                                            <td><?php echo ($vo["account_name"]); ?></td>
                                            <td>
                                                <?php if($vo['status'] == 0): ?>申请中<?php endif; ?>                                        
                                                <?php if($vo['status'] == 1): ?>申请成功<?php endif; ?>
                                                <?php if($vo['status'] == 2): ?>申请失败<?php endif; ?>
                                            </td>
                                            <td>
                                                <a href="<?php echo U('user/editWithdraw',array('id'=>$vo['id']));?>">编辑</a>
                                                <?php if(in_array($vo['status'],array(0,2))): ?><span class="text-explode">|</span>
                                                    <a href="<?php echo U('user/delWithdraw',array('id'=>$vo['id']));?>">删除</a><?php endif; ?>
                                            </td>
                                        </tr><?php endforeach; endif; ?>
                                </tbody>
                                <tfoot>
                                    <tr>
                                        <td colspan="9" align="right"><?php echo ($list["page"]); ?></td>
                                    </tr>
                                </tfoot>
                            <?php else: ?>
                                <tr>
