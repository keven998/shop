                        ...
                        <table class="table  table-header">
                            ...
                            <tbody>
                                <?php if(is_array($list)): foreach($list as $key=>$vo): ?><tr>
                                            ...
                                            <td><?php if($vo['last_login']): echo (date("Y-m-d H:i:s",$vo["last_login"])); else: ?>未登录过<?php endif; ?></td>
                                            <td><?php if($vo['last_ip']): echo ($vo["last_ip"]); else: ?>未登录过<?php endif; ?></td>
                                            <td>
                                                <?php if(!$vo['lock']): ?>正常<?php else: ?>冻结<?php endif; ?>
                                            </td>
                                            <td>
                                                <?php if($vo['username'] != C('RBAC_SUPERADMIN')): ?><a href="<?php echo U('admin/editUser', array('id'=>$vo['user_id']));?>">修改</a>
                                                    <span class="text-explode">|</span>
                                                    <?php if($vo['lock']): ?><a href="<?php echo U('admin/lock', array('id'=>$vo['user_id'],'lock'=>0));?>">激活</a><?php else: ?><a href="<?php echo U('admin/lock', array('id'=>$vo['user_id'],'lock'=>1));?>">冻结</a><?php endif; ?>
                                                    <span class="text-explode">|</span>
                                                    <a href="<?php echo U('admin/delUser', array('id'=>$vo['user_id']));?>">删除</a><?php endif; ?>
                                            </td>
